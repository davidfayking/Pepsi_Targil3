...
def runge_kutta_rocket_with_random_and_correction(r_0=(0, 0, 0), v_0=500, dt=0.1, N=10000, m=500, theta=np.radians(20),
    ...
    correction = (0, 0)
    theoretical_t, theoretical_r, _ = helper.runge_kutta_rocket(dt=delta_t, N=n)
    for i in range(N - 1):
        ...
        correction = calculate_correction(current_position,current_velocity,theoretical_r,dt)
        if (z[i + 1] < 0):
            break
...
target_radius = 10
def calculate_correction(current_pos, current_velocity,theoretical_pos, dt =0.1, target_position=(target_x, target_y)):
    # return (0, 0)
    """
